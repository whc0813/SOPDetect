...
const openCreateDialog = () => {
  ...
  sopForm.scene = ''
  sopForm.stepCount = 1
  sopForm.steps = [{ description: '', video: null }]
...
const handleStepCountChange = (newVal) => {
  ...
  if (newVal > currentLen) {
    for (let i = currentLen; i < newVal; i++) {
      sopForm.steps.push({ description: '', video: null })
    }
...
const handleStepVideoChange = (index, file) => {
  sopForm.steps[index].video = { name: file.name }
}
...
const saveSop = () => {
  ...
  const newSop = {
    ...
    steps: sopForm.steps.map(step => ({
      description: step.description,
      video: step.video ? { name: step.video.name } : null
    }))
  }
      ...
      ${row.steps.map((s, i) => `
        <div style="margin-bottom: 12px; padding: 12px; background: #f5f5f7; border-radius: 8px;">
          <div style="font-size: 12px; color: #86868b; margin-bottom: 8px; display: flex; justify-content: space-between; align-items: center;">
            <span>步骤 ${i+1}</span>
            ${s.video ? `<span style="color: #1d1d1f; font-weight: 500; display: flex; align-items: center; gap: 4px;"><svg viewBox="0 0 1024 1024" width="14" height="14"><path d="M512 64a448 448 0 1 1 0 896 448 448 0 0 1 0-896z m0 832a384 384 0 0 0 0-768 384 384 0 0 0 0 768z m-96-544l256 160-256 160V352z" fill="currentColor"></path></svg> ${s.video.name}</span>` : ''}
          </div>
